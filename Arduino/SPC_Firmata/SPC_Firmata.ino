...
String macAddress;
const int STATUS_ERR = 22;
const int STATUS_OK = 20;
       ...
       // TODO! UNSAFE! more than 50 characters can cause crashes!
        ...
        for(unsigned int i=0;i<50;i++){
          if(argv[i]==';'){
            ...
            wifi_ssid_str = wifi_ssid_str + (char)argv[i];
          } else if(param==2){
            wifi_pwd_str = wifi_pwd_str + (char)argv[i];
          } else if(param==3){
            label = label + (char)argv[i];
          } else if(param==4){
            break;
          }
        }

        initDone = true; //setupWifi...
      }
...
/*****UTILITY FUNCTIONS!****/
/*
 *Check weight of the container.
 *Return 1 if the weight is different from the last read
 *Return 0 if the weight is the same of the last read
 */
int updateWeight() {
...
int setupWifi(){
  if( wifi_ssid_str.length() == 0 || wifi_pwd_str.length() == 0 || 
    ...
    messageToLCD("Wifi: Invalid","Authentication");
    return 0;
  }
  
  char wifi_ssid[30]; //TODO UNSAFE
  char wifi_pwd[30]; //TODO UNSAFE
  wifi_ssid_str.toCharArray(wifi_ssid, wifi_ssid_str.length()+1);
  wifi_pwd_str.toCharArray(wifi_pwd, wifi_pwd_str.length()+1);
  //useful for debugging messageToLCD(wifi_ssid,wifi_pwd);
  delay(3000);
  messageToLCD("Initializing..","Wifi");
  int wifi_status = WiFi.begin(wifi_ssid,wifi_pwd);

  int retry = 10;
  while ( wifi_status != WL_CONNECTED) {
    status = STATUS_ERR;
    messageToLCD("Connecting..","Wifi");
    delay(1000);
    retry --;
    if(retry == 0) {
      messageToLCD("Initializing..","Wifi: Failed");
      delay(3000);
      return 0;
    }
  }

  messageToLCD("Initializing..","Wifi DONE");
  delay(2000);  
  lastUpdate = millis();
...
/*
 *Save the macAddress in a string.
 */
void readMacAddress() {
  byte mac[6];
  WiFi.macAddress(mac);
...
/*
 * update the 2 line of the lcd.
 * */
void messageToLCD(char *msgLine1,char *msgLine2) {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print(msgLine1);
  lcd.setCursor(0, 1);
